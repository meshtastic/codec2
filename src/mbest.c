...
void mbest_search(
		  const int8_t *cb,      /* VQ codebook to search (int8)  */
...
)
{
    ...
    */
    ...
    for(j=0; j<m; j++) {
        float e = 0.0;
        for(int i=0; i<k; i++) {
	    float diff = CODEBOOK_Q8(cb, 0) - vec[i];
            cb++;
            e += diff*diff;
        }
...
void mbest_search450(const int8_t *cb, float vec[], float w[], int k,int shorterK, int m, struct MBEST *mbest, int index[])

{
	...
	for(i=0; i<k; i++) {
            ...
            if(i<shorterK){
                diff = CODEBOOK_Q8(cb, j*k+i)-vec[i];
                e += diff*w[i] * diff*w[i];
            }
	}
	index[0] = j;
