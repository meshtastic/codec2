...
{
  int i, j, n1, n2;
  const int8_t *codebook1 = newamp1vq_cb[0].cb;
  const int8_t *codebook2 = newamp1vq_cb[1].cb;
  ...
  for (j=0; j<mbest_entries; j++) {
      ...
      for(i=0; i<ndim; i++)
	  target[i] = x[i] - CODEBOOK_Q8(codebook1, ndim*n1+i);
  ...
  mse = 0.0;
  for (i=0;i<ndim;i++) {
      tmp = CODEBOOK_Q8(codebook1, ndim*n1+i) + CODEBOOK_Q8(codebook2, ndim*n2+i);
      mse += (x[i]-tmp)*(x[i]-tmp);
      xq[i] = tmp;
  }
                                   ...
                                   int post_filter_en)
{
    ...
    if (user_rate_K_vec_no_mean_ == NULL) {
        /* normal operation */
        for(k=0; k<K; k++) {
            rate_K_vec_no_mean_[k] = CODEBOOK_Q8(codebook1, K*n1+k) + CODEBOOK_Q8(codebook2, K*n2+k);
        }
    } else {
