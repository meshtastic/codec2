...
{
  int i, n1;
  const int8_t *codebook1 = newamp2vq_cb[0].cb;
                                   ...
                                   float  pf_gain)
{
    ...
    for(k=0; k<K; k++) {
      rate_K_vec_no_mean_[k] = CODEBOOK_Q8(codebook1, (NEWAMP2_16K_K+1)*n1+k);
    }
                                   ...
                                   float  pf_gain)
{
    ...
    for(k=0; k<K; k++) {
      rate_K_vec_no_mean_[k] = CODEBOOK_Q8(codebook1, (K+1)*n1+k);
    }

    n2_post_filter_newamp2(rate_K_vec_no_mean_, rate_K_sample_freqs_kHz, K, pf_gain);

    *mean_ = newamp2_energy_cb[0].cb[indexes[2]];
    mean2 = *mean_  + CODEBOOK_Q8(codebook1, (K+1)*n1+K) -10;
